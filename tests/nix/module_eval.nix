...
assert cfg.services.dovecot2.settings.auth_allow_cleartext;
assert cfg.services.dovecot2.settings.service != [ ];
assert (builtins.elemAt cfg.services.dovecot2.settings.service 0)."inet_listener imap".port == 143;
...
assert cfg.networking.firewall.interfaces.tailscale0.allowedTCPPorts == [ 143 465 993 ];
